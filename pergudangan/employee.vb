Public Class employee
    ...
    Public Sub loadZX()
        dgv_list.DataSource = getDataToDataGrid("select employee_id, employee_name from employee")
    ...
    Private Sub dgv_list_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_list.CellContentClick
        ...
        If dgv_list.Columns(e.ColumnIndex).Name = "btn_edit" Then
            employee_add_edit.MdiParent = main_menu
            employee_add_edit.Show()
            employee_add_edit.loadParams(dgv_list.CurrentRow.Cells("employee_id").Value)
        ElseIf dgv_list.Columns(e.ColumnIndex).Name = "btn_delete" Then
            Dim result As DialogResult = MessageBox.Show("Delete " & dgv_list.CurrentRow.Cells("employee_name").Value & "?", "Are you sure?", MessageBoxButtons.YesNo)
            If result = DialogResult.Yes Then
                getExecute("DELETE FROM employee WHERE employee_id = " & dgv_list.CurrentRow.Cells("employee_id").Value)
    ...
    Private Sub ms_add_Click(sender As Object, e As EventArgs) Handles ms_add.Click
        employee_add_edit.Show()
        employee_add_edit.loadParams(0)
    End Sub
